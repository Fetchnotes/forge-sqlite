...
def get_or_ask_for_local_config(build, required_info, question_title):
	...
	for dotted_name, schema in required_info.iteritems():
		if dotted_name in build.tool_config:
			value = build.tool_config[dotted_name]
			allow_blanks = schema.get('blank', False)

			if not allow_blanks and value == "":
				unknown_info[dotted_name] = required_info[dotted_name]
			else:
				known_info[dotted_name] = value
		else:
			unknown_info[dotted_name] = required_info[dotted_name]
