...
//     */
    ...
    asyncTest('can search for text with multiple strings', function() {
      var search;
      search = ['book', 'via', 'read'];
      return $db.getNotes({
        search: search
      }).then(function (notesFromDb) {
        var query, _i, _len;
        equal(notesFromDb.length, 1);
        for (_i = 0, _len = search.length; _i < _len; _i++) {
          query = search[_i];
          notEqual(notesFromDb[0].text.indexOf(query), -1);
        }
        start();
      });
    });
   ...
   asyncTest('can count all notes, then stashed and unstashed', function () {
    	$db.countNotes().then(function (count) {
    		equal(count, 8);
        return $db.countNotes({stashed: true});
    	}).then(function (count) {
        equal(count, 2);
        return $db.countNotes({stashed: false});
      }).then(function (count) {
        equal(count, 6);
        start();
      });
   ...
   asyncTest('verifies dirty note and search counts', function () {
    $db.countNotes({dirty: true}).then(function (count) {
      equal(count, 2);
      return $db.countNotes({search: ['via', 'book', 'read']});
    }).then( function (count) {
      equal(count, 1);
      return $db.countNotes({search: 'book'})
    }).then( function (count) {
      equal(count, 2);
      return $db.countNotes({search: 'lulzlulz'});
    }).then(function (count) {
      equal(count, 0);
      start();
    })
   ...
   asyncTest('verifies counts by hashtags', function () {
      $db.countNotes({attags: ['@lubibul']}).then(function (count) {
        equal(count, 2);
        return $db.countNotes({hashtags: ['#todo']});
      }).then(function (count) {
        equal(count, 2);
        return $db.countNotes({attags: ['@butts']});
      }).then(function (count) {
        equal(count, 0);
        start();
      })
